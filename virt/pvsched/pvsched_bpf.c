				  ...
				  void *kdata, const void *udata)
{
	...
	struct pvsched_vcpu_ops *kvm_ops;
	u32 moff;

	uvm_ops = (const struct pvsched_vcpu_ops *)udata;
	kvm_ops = (struct pvsched_vcpu_ops *)kdata;

	moff = __btf_member_bit_offset(t, member) / 8;
	switch (moff) {
	case offsetof(struct pvsched_vcpu_ops, events):
		kvm_ops->events = *(u32 *)(udata + moff);
		return 1;
	case offsetof(struct pvsched_vcpu_ops, name):
		if (bpf_obj_name_cpy(kvm_ops->name, uvm_ops->name,
					sizeof(kvm_ops->name)) <= 0)
			return -EINVAL;
		return 1;
	}
